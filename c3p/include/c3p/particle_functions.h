...
//  Particle particle(float);
  ...
  Force calc_force(const NObject & p1, const NObject & p2, std::function<Force(const Particle &, const Particle &)> ff);
  ...
  //calculates the force between two particles if they were attached to a spring (initializer list is for Spring constructor)
  Force spring(const Particle & p1, const Particle & p2, std::initializer_list<float> G);
