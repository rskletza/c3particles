 ...
 public:
  class Iterator
  {
   public:
    ...
    const reference operator*() const { return _container[_index]; }
    reference operator*() { return _container[_index]; }
    const reference operator[](int offset) const { return *(*this + offset); }
    reference operator[](int offset) { return *(*this + offset); }
    iterator &operator++()
    {
      ++_index;
      return *this;
    ...
    iterator &operator+(int offset) const { return (*this)[offset]; }
    iterator &operator-(int offset) const { return (*this)[-offset]; }
  ...
  ~ParticleContainer() { delete[] _elements; }
  ...
  bool operator!=(self_t &rhs) const { return !(*this != rhs); }
  bool empty() const { return _elements->empty(); }
  bool size() const { return _elements->size(); }
  template <class T>
  void add(T &&object)
  {
