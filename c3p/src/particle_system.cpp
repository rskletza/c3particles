...
ParticleSystem::ParticleSystem(size_t size)
    : _G(1.0 *
         std::pow(
             10, -4)) // universal gravitational constant (actually 6.67*10^-11)
{
...
void ParticleSystem::update() {
  ...
  for (Particle &p : *_particles) {
    ...
    // point in time
    //--> v(t) = a*t + v(t-1)
    // t in this case is equal to 1, as I am using frames to measure time (TODO
...
void ParticleSystem::print() const {
  for (auto p : *_particles) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      //            std::endl; std::cout << "v " << i << " " << p.velocity[i] <<
      //            std::endl;
